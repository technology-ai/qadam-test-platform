...
const accounts = { admin: { password: 'admin123', name: 'Администратор', role: 'admin' }, user: { password: 'user123', name: 'Пользователь', role: 'user' } };
let currentUser = store.get('suist20.user', null);
const demo = [
...
function render() {
  ...
  app.innerHTML = `<header class="top"><div class="logo"><span>С</span>СУИСТ 2.0</div><small>${currentUser.name} · ${currentUser.role === 'admin' ? 'Администратор' : 'Пользователь'} &nbsp; <button class="ghost" id="logout" style="color:#fff">Выйти</button></small></header>
  ...
  document.querySelector('#logout').onclick = () => { currentUser = null; store.set('suist20.user', null); render(); };
  drawRows(); bind();
}
function renderLogin() {
  app.innerHTML = `<main class="wrap" style="max-width:440px;margin-top:11vh"><section class="card"><div class="logo" style="color:var(--navy)"><span>С</span>СУИСТ 2.0</div><h1 style="margin:25px 0 4px">Вход в систему</h1><p class="muted">Введите учётные данные для продолжения работы.</p><form id="login-form" class="form" style="margin-top:22px"><label class="field full">Логин<input required name="login" autocomplete="username" placeholder="Введите логин"></label><label class="field full">Пароль<input required type="password" name="password" autocomplete="current-password" placeholder="Введите пароль"></label><div class="field full" id="login-error" style="color:var(--red)"></div><button class="primary field full">Войти</button></form><p class="notice">Демо-доступ: <b>admin / admin123</b><br>Пользователь: <b>user / user123</b></p></section></main>`;
  document.querySelector('#login-form').onsubmit = e => { e.preventDefault(); const d = new FormData(e.target), a = accounts[d.get('login')]; if (!a || a.password !== d.get('password')) { document.querySelector('#login-error').textContent = 'Неверный логин или пароль.'; return; } currentUser = { name:a.name, role:a.role }; store.set('suist20.user', currentUser); render(); };
}
function drawRows() {
  ...
  document.querySelector('#rows').innerHTML=list.length ? list.map((p,i)=>{const s=state(p), canEdit=currentUser.role==='admin'||p.owner===currentUser.name; return `<tr><td>${i+1}</td><td><b>${esc(p.name)}</b><br><span class="pill ${s[1]}">${s[0]}</span></td><td>${date(p.due)}</td><td>${esc(p.owner)}</td><td>${esc(p.comment)}</td><td>${p.photo?`<img class="thumb" data-photo="${p.id}" src="${p.photo}" alt="Фото">`:'<div class="no-photo">нет</div>'}</td><td><b>${p.percent}%</b><div class="progress"><i style="width:${p.percent}%"></i></div></td><td>${canEdit?`<button class="ghost" data-edit="${p.id}">Изменить</button>${currentUser.role==='admin'?`<button class="ghost" data-delete="${p.id}">Удалить</button>`:''}`:'<span class="muted">Только просмотр</span>'}</td></tr>`}).join('') : '<tr><td colspan="8" class="empty">По вашему запросу позиции не найдены.</td></tr>';
...
function openForm(id) {
  const p=positions.find(x=>x.id===id)||{name:'',due:'',owner:currentUser.role==='user'?currentUser.name:'',comment:'',percent:0,photo:''}; if (id && currentUser.role!=='admin' && p.owner!==currentUser.name) return; pendingPhoto=p.photo||''; activePhotoId=id||null;
  modal(`<h2>${id?'Редактировать':'Новая'} позиция</h2><form id="position-form" class="form"><label class="field full">Наименование позиции<input required name="name" value="${esc(p.name)}" placeholder="Введите наименование"></label><label class="field">Срок выполнения<input required type="date" name="due" value="${p.due}"></label><label class="field">Исполнитель<input required name="owner" value="${esc(p.owner)}" placeholder="ФИО или должность" ${currentUser.role==='user'?'readonly':''}></label><label class="field">Процент выполнения<input required type="number" min="0" max="100" name="percent" value="${p.percent}"></label><label class="field">Фото<button type="button" class="secondary" id="choose-photo">Загрузить фото</button>${p.photo?'<button type="button" class="danger" id="remove-photo">Удалить фото</button>':''}<img class="photo-preview" id="photo-preview" ${p.photo?'src="'+p.photo+'"':'hidden'}></label><label class="field full">Комментарий<textarea name="comment" rows="3" placeholder="Комментарий">${esc(p.comment)}</textarea></label></form><div class="modal-actions"><button class="secondary" id="cancel">Отмена</button><button class="primary" id="save-position">Сохранить</button></div>`);
  ...
  const removePhoto = document.querySelector('#remove-photo'); if (removePhoto) removePhoto.onclick=()=>{ pendingPhoto=''; document.querySelector('#photo-preview').hidden=true; removePhoto.remove(); };
...
document.querySelector('#excel-file').onchange=e=>{const file=e.target.files[0];if(!file)return;if(!window.XLSX){alert('Библиотека Excel ещё загружается.');return;}const r=new FileReader();r.onload=ev=>{try{const wb=XLSX.read(ev.target.result,{type:'array'}),sheet=wb.Sheets[wb.SheetNames[0]],rows=XLSX.utils.sheet_to_json(sheet,{defval:''});const fields=['Наименование позиции','Срок выполнения','Исполнитель'];if(!rows.length||!fields.every(f=>Object.hasOwn(rows[0],f))){alert('Нужны столбцы: Наименование позиции, Срок выполнения, Исполнитель.');return;}const imported=rows.map((r,i)=>({id:(Date.now()+i).toString(),name:String(r['Наименование позиции']).trim(),due:excelDate(r['Срок выполнения']),owner:String(r['Исполнитель']).trim(),comment:String(r['Комментарий']||'').trim(),percent:Math.min(100,Math.max(0,Number(r['Процент выполнения'])||0)),photo:''})).filter(p=>p.name&&p.owner&&p.due);previewImport(imported);}catch{alert('Не удалось прочитать файл Excel.');}};r.readAsArrayBuffer(file);e.target.value='';};
function previewImport(items) { modal(`<h2>Предпросмотр импорта</h2><p class="notice">Найдено корректных позиций: <b>${items.length}</b>. После подтверждения они будут добавлены в таблицу.</p><div style="max-height:270px;overflow:auto"><table class="table" style="min-width:500px"><thead><tr><th>Наименование</th><th>Срок</th><th>Исполнитель</th><th>%</th></tr></thead><tbody>${items.slice(0,8).map(p=>`<tr><td>${esc(p.name)}</td><td>${date(p.due)}</td><td>${esc(p.owner)}</td><td>${p.percent}%</td></tr>`).join('')}${items.length>8?`<tr><td colspan="4">… ещё ${items.length-8} позиций</td></tr>`:''}</tbody></table></div><div class="modal-actions"><button class="secondary" id="cancel-import">Отмена</button><button class="primary" id="confirm-import">Импортировать</button></div>`); document.querySelector('#cancel-import').onclick=closeModal;document.querySelector('#confirm-import').onclick=()=>{positions.push(...items);closeModal();save();}; }
